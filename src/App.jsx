...
const A="#10B981",A2="#059669",A3="#34D399",AG="rgba(16,185,129,0.10)",AS="rgba(16,185,129,0.05)";
const BG="#000000",BG2="#111111",CARD="#121212",CARD2="#1C1C1C",BD="#262626",BD2="#363636";
...
// ═══ SHARED COMPONENTS ═══
function Logo({size="default"}){const s=size==="small"?28:36;return el("div",{style:{display:"flex",alignItems:"center",gap:10}},el("div",{className:"logo-pulse",style:{width:s,height:s,borderRadius:s*.28,background:`linear-gradient(135deg,${A},${A2})`,display:"flex",alignItems:"center",justifyContent:"center"}},IC.book("#fff",s*.48)),el("span",{style:{fontSize:size==="small"?14:17,fontWeight:700,color:T1,letterSpacing:-.5}},"StudyFlow"));}
...
// ═══ STUDY TIMER ═══
function TimerScreen({sessions,onSession}){
  ...
  const stop=()=>{if(seconds>0){onSession({duration:seconds,ts:Date.now()});logEvent("timer_session",{seconds});}setRunning(false);setSeconds(0);};
  const totalToday=sessions.reduce((s,x)=>s+x.duration,0);
  const CIRC=2*Math.PI*90;
  const goalSecs=3600;
  const dashOffset=CIRC*(1-Math.min(seconds/goalSecs,1));

  return el("div",{style:S.pad},el(Stagger,{delay:60},
    el("h2",{style:{fontSize:20,fontWeight:700,color:T1,marginBottom:24}},"공부 타이머"),
    el("div",{style:{textAlign:"center",padding:"40px 0",marginBottom:24}},
      el("div",{style:{position:"relative",width:200,height:200,margin:"0 auto",marginBottom:32}},
        el("svg",{width:200,height:200,style:{transform:"rotate(-90deg)",position:"absolute",inset:0}},
          ...
          (running||seconds>0)&&el("circle",{cx:100,cy:100,r:90,fill:"none",stroke:A,strokeWidth:6,strokeDasharray:CIRC,strokeDashoffset:dashOffset,strokeLinecap:"round",style:{transition:"stroke-dashoffset 1s linear"}})
        ),
        el("div",{style:{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}},
          el("span",{style:{fontSize:44,fontWeight:700,color:T1,fontVariantNumeric:"tabular-nums",letterSpacing:-1}},fmt(seconds)),
          el("span",{style:{fontSize:12,color:T3,marginTop:4}},running?"집중 중...":"준비")
        )
      ),
      el("div",{style:{display:"flex",justifyContent:"center",gap:16}},
        !running?el("button",{onClick:()=>{setRunning(true);logEvent("timer_start");},style:{width:64,height:64,borderRadius:32,background:`linear-gradient(135deg,${A},${A2})`,border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}},IC.play("#fff",24)):
    ...
    el("div",{style:{background:CARD,border:`1px solid ${BD}`,borderRadius:16,padding:18,marginBottom:16}},
      el("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center"}},
        ...
        el("span",{style:{fontSize:20,fontWeight:700,color:A}},fmt(totalToday))
      )
    ),
  ...
  return el("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",backdropFilter:"blur(12px)",zIndex:200,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,animation:"fadeIn 0.2s"},onClick:onClose},
    el("div",{style:{width:"100%",maxWidth:300,animation:"popIn 0.3s cubic-bezier(0.34,1.56,0.64,1)"},onClick:e=>e.stopPropagation()},
      ...
      el("div",{style:{display:"flex",gap:10,marginTop:16}},
        el("button",{onClick:onClose,style:{flex:1,background:CARD,border:`1px solid ${BD}`,borderRadius:22,color:T2,padding:14,fontSize:14,fontWeight:600,cursor:"pointer"}},"닫기"),
...
function PaywallModal({onClose,onUpgrade}){
  ...
  return el("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",backdropFilter:"blur(10px)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20,animation:"fadeIn 0.2s"},onClick:onClose},
    el("div",{style:{width:"100%",maxWidth:380,background:CARD,borderRadius:20,padding:"28px 24px",border:`1px solid ${BD}`,animation:"popIn 0.35s cubic-bezier(0.34,1.56,0.64,1)",textAlign:"center",maxHeight:"90vh",overflowY:"auto"},onClick:e=>e.stopPropagation()},
      el("div",{style:{width:56,height:56,borderRadius:16,background:`linear-gradient(135deg,${WARN},#F97316)`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px"}},IC.crown("#fff",24)),
      el("h3",{style:{fontSize:18,fontWeight:700,color:T1,marginBottom:6}},"StudyFlow Pro"),
      el("p",{style:{fontSize:13,color:T2,lineHeight:1.6,marginBottom:20}},"시험 직전 복습 플랜과 프리미엄 기능을 잠금 해제하세요"),
      el("div",{style:{background:BG2,borderRadius:22,padding:14,marginBottom:16,textAlign:"left"}},
      ...
      el("div",{style:{marginBottom:14}},el("span",{style:{fontSize:28,fontWeight:700,color:T1}},"₩4,900"),el("span",{style:{fontSize:13,color:T3}},"/월")),
      ...
      el("button",{onClick:parentRequest,style:{width:"100%",background:"rgba(245,158,11,0.06)",border:`1.5px solid rgba(245,158,11,0.15)`,borderRadius:22,padding:"13px",display:"flex",alignItems:"center",justifyContent:"center",gap:8,cursor:"pointer",marginBottom:8}},
...
// ═══ WHEEL PICKER ═══
function WheelPicker({options,selected,onChange}){
  const ref=useRef(null);
  const IH=44;
  useEffect(()=>{
    const idx=options.indexOf(selected);
    if(ref.current&&idx>=0)ref.current.scrollTop=idx*IH;
  },[]);
  const onScroll=()=>{
  ...
  return el("div",{style:{position:"relative",height:IH*3,flex:1,background:BG2,borderRadius:16,overflow:"hidden"}},
    el("div",{style:{position:"absolute",top:IH,height:IH,left:4,right:4,background:AS,border:`1px solid ${BD2}`,borderRadius:10,pointerEvents:"none",zIndex:1}}),
    el("div",{style:{position:"absolute",top:0,height:Math.round(IH*.7),left:0,right:0,background:`linear-gradient(to bottom,${BG2},transparent)`,pointerEvents:"none",zIndex:2}}),
    el("div",{style:{position:"absolute",bottom:0,height:Math.round(IH*.7),left:0,right:0,background:`linear-gradient(to top,${BG2},transparent)`,pointerEvents:"none",zIndex:2}}),
    el("div",{ref,onScroll,style:{height:"100%",overflowY:"scroll",scrollSnapType:"y mandatory",scrollbarWidth:"none",msOverflowStyle:"none",paddingTop:IH,paddingBottom:IH,WebkitOverflowScrolling:"touch"}},
      options.map((opt,i)=>el("div",{key:i,onClick:()=>{if(ref.current)ref.current.scrollTop=i*IH;onChange(opt);},style:{height:IH,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,fontWeight:opt===selected?700:400,color:opt===selected?T1:T3,scrollSnapAlign:"center",userSelect:"none",cursor:"pointer",transition:"color 0.15s"}},opt))
    )
  );
}

// ═══ EDIT MODAL ═══
function EditModal({task,onSave,onDelete,onClose}){const[t,setT]=useState(task.task);const[s,setS]=useState(task.subject);const parseDur=str=>{const mx=str.match(/(\d+)h\s*(\d+)m/);if(mx)return{h:mx[1]+"h",m:String(parseInt(mx[2])).padStart(2,"0")+"m"};const n=parseFloat(str);if(!isNaN(n)){const h=Math.floor(n);const mi=Math.min(Math.round((n-h)*4)*15,45);return{h:h+"h",m:String(mi).padStart(2,"0")+"m"};}return{h:"1h",m:"00m"};};const ini=parseDur(task.duration);const[dh,setDh]=useState(ini.h);const[dm,setDm]=useState(ini.m);const HOURS=["0h","1h","2h","3h","4h","5h"];const MINS=["00m","15m","30m","45m"];const duration=dh.replace("h","")+"h "+dm.replace("m","").padStart(2,"0")+"m";return el("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(8px)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center",animation:"fadeIn 0.2s"},onClick:onClose},el("div",{style:{width:"100%",maxWidth:480,background:CARD,borderRadius:"20px 20px 0 0",padding:"24px 20px 36px",border:`1px solid ${BD}`,borderBottom:"none",animation:"slideUp 0.35s cubic-bezier(0.16,1,0.3,1)"},onClick:e=>e.stopPropagation()},el("div",{style:{width:36,height:4,borderRadius:2,background:BD2,margin:"0 auto 20px"}}),el("h3",{style:{fontSize:16,fontWeight:700,color:T1,marginBottom:20}},"태스크 수정"),el("div",{style:{marginBottom:14}},el("label",{style:S.label},"과목"),el("input",{value:s,onChange:e=>setS(e.target.value),style:S.input,className:"input-glow"})),el("div",{style:{marginBottom:14}},el("label",{style:S.label},"내용"),el("textarea",{value:t,onChange:e=>setT(e.target.value),rows:3,style:S.inputArea,className:"input-glow"})),el("div",{style:{marginBottom:20}},el("label",{style:S.label},"소요 시간"),el("div",{style:{display:"flex",gap:10}},el(WheelPicker,{options:HOURS,selected:dh,onChange:setDh}),el(WheelPicker,{options:MINS,selected:dm,onChange:setDm}))),el("div",{style:{display:"flex",gap:10}},el("button",{onClick:()=>{logEvent("task_deleted",{id:task.id});onDelete(task.id);},style:{flex:1,background:"rgba(239,68,68,0.1)",border:"1.5px solid rgba(239,68,68,0.2)",borderRadius:22,color:DANGER,padding:13,fontSize:14,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}},IC.trash(DANGER,14)," 삭제"),el("button",{onClick:()=>{logEvent("task_edited",{id:task.id});onSave({...task,task:t,duration:duration,subject:s});},className:"btn-glow",style:{flex:2,...S.genBtn,padding:13,fontSize:14}},"저장"))));}

// ═══ LOADING ═══
function LoadingScreen(){const steps=[{l:"시험 범위 분석",ic:IC.target},{l:"학습 프로필 반영",ic:IC.zap},{l:"일정 최적화",ic:IC.sparkle},{l:"플랜 생성 완료",ic:IC.check}];const[step,setStep]=useState(0);const[dots,setDots]=useState("");useEffect(()=>{const t=setInterval(()=>setStep(p=>Math.min(p+1,3)),1800);return()=>clearInterval(t);},[]);useEffect(()=>{const t=setInterval(()=>setDots(p=>p.length>=3?"":p+"."),400);return()=>clearInterval(t);},[]);return el("div",{style:{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",padding:40,position:"relative",overflow:"hidden"}},el("div",{style:{position:"relative",width:120,height:120,marginBottom:48}},el("div",{className:"ring1",style:{position:"absolute",inset:0,borderRadius:"50%",border:`2px solid ${BD}`,borderTopColor:A}}),el("div",{className:"ring2",style:{position:"absolute",inset:12,borderRadius:"50%",border:`2px solid ${BD}`,borderRightColor:A3,opacity:.7}}),el("div",{className:"ring3",style:{position:"absolute",inset:24,borderRadius:"50%",border:`2px solid ${BD}`,borderBottomColor:A2,opacity:.5}}),el("div",{style:{position:"absolute",inset:36,borderRadius:"50%",background:`linear-gradient(135deg,${A},${A2})`,display:"flex",alignItems:"center",justifyContent:"center"}},IC.book("#fff",24))),el("div",{style:{width:"100%",maxWidth:280,zIndex:1}},steps.map((s,i)=>{const ac=i===step,dn=i<step;return el("div",{key:i,style:{display:"flex",alignItems:"center",gap:14,padding:"12px 0",opacity:dn?.4:ac?1:.2,transition:"all 0.5s",transform:ac?"translateX(4px)":"none"}},el("div",{style:{width:32,height:32,borderRadius:10,background:dn?A:ac?AS:"transparent",border:`1.5px solid ${dn?A:ac?A:BD}`,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:ac?`0 0 20px ${AG}`:"none"}},dn?IC.check("#fff",14):s.ic(ac?A:T4,16)),el("span",{style:{fontSize:14,fontWeight:ac?700:500,color:dn?T3:ac?T1:T4}},s.l+(ac?dots:"")));})),el("div",{style:{width:"100%",maxWidth:280,marginTop:24}},el("div",{style:{height:3,background:BD,borderRadius:2,overflow:"hidden"}},el("div",{style:{height:"100%",borderRadius:2,background:`linear-gradient(90deg,${A2},${A},${A3})`,width:`${((step+1)/4)*100}%`,transition:"width 0.8s"}}))));}

// ═══ NAV ═══
function BottomNav({active,onNav}){const items=[{id:"home",l:"홈",icon:IC.home},{id:"plan",l:"플랜",icon:IC.plan},{id:"timer",l:"타이머",icon:IC.timer},{id:"calendar",l:"캘린더",icon:IC.cal},{id:"settings",l:"설정",icon:IC.settings}];return el("div",{className:"sf-nav",style:S.nav},el("div",{style:S.navInner},items.map(it=>{const on=active===it.id;return el("button",{key:it.id,onClick:()=>{logEvent("tab_switch",{tab:it.id});onNav(it.id);},style:{...S.navItem,padding:"6px 12px"}},el("div",{style:{transition:"all 0.2s ease",transform:on?"scale(1.08)":"none"}},it.icon(on?T1:T4,20)),el("span",{style:{fontSize:9,color:on?T1:T4,fontWeight:on?600:500,opacity:on?1:.6}},it.l));})));}

// ═══ TASK ITEM ═══
function TaskItem({task,onToggle,onEdit,locked}){const[pop,setPop]=useState(false);const click=()=>{if(locked)return;if(!task.done)setPop(true);logEvent("task_checked",{id:task.id});onToggle();setTimeout(()=>setPop(false),400);};if(locked)return el("div",{style:{display:"flex",alignItems:"center",gap:12,padding:"13px 14px",background:CARD,border:`1px solid ${BD}`,borderRadius:22,position:"relative",overflow:"hidden"}},el("div",{style:{position:"absolute",inset:0,background:"rgba(5,5,6,0.7)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1,borderRadius:22}},IC.lock(T3,20)),el("div",{style:{flex:1,opacity:.3}},el("span",{style:{fontSize:10,fontWeight:700,color:A}},task.subject),el("br"),el("span",{style:{fontSize:13,color:T3}},task.task)));return el("div",{style:{display:"flex",alignItems:"center",gap:12,padding:"13px 14px",background:CARD,border:`1px solid ${BD}`,borderRadius:22,cursor:"pointer",transition:"all 0.25s",opacity:task.done?.4:1,transform:pop?"scale(0.97)":"scale(1)"}},el("div",{onClick:click,style:{width:22,height:22,minWidth:22,borderRadius:7,border:`2px solid ${task.done?A:BD2}`,background:task.done?A:"transparent",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.3s cubic-bezier(0.34,1.56,0.64,1)",transform:pop?"scale(1.2)":"scale(1)"}},task.done&&el("div",{style:{animation:"popIn 0.3s cubic-bezier(0.34,1.56,0.64,1)"}},IC.check("#fff",12))),el("div",{onClick:click,style:{flex:1,display:"flex",flexDirection:"column",gap:3}},el("span",{style:{fontSize:10,fontWeight:700,color:A,textTransform:"uppercase",letterSpacing:.5}},task.subject),el("span",{style:{fontSize:13,color:"#d4d4d8",lineHeight:1.4,textDecoration:task.done?"line-through":"none"}},task.task)),el("div",{style:{display:"flex",alignItems:"center",gap:8,flexShrink:0}},el("div",{style:{display:"flex",alignItems:"center",gap:4}},IC.clock(T4,12),el("span",{style:{fontSize:11,color:T4}},task.duration)),el("button",{onClick:e=>{e.stopPropagation();onEdit(task);},style:{background:"none",border:"none",cursor:"pointer",padding:4,opacity:.5}},IC.edit(T2,14))));}

// ═══ HOME ═══
function HomeScreen({plan,onToggle,onNav,onEdit,onShare,streak,timerSecs=0}){const today=new Date().toISOString().split("T")[0];const tp=plan?plan.find(d=>d.date===today):null;const total=plan?plan.reduce((s,d)=>s+d.tasks.length,0):0;const done=plan?plan.reduce((s,d)=>s+d.tasks.filter(t=>t.done).length,0):0;const pct=total>0?Math.round((done/total)*100):0;const di=plan?plan.findIndex(d=>d.date===today):-1;const td=tp?tp.tasks.filter(t=>t.done).length:0;const tt=tp?tp.tasks.length:0;const tp2=tt>0?Math.round((td/tt)*100):0;const planTodaySecs=tp?tp.tasks.reduce((s,t)=>s+parseHMtoSecs(t.duration),0):0;const timerPct=planTodaySecs>0?Math.min(100,Math.round(timerSecs/planTodaySecs*100)):0;const ringPct=timerSecs>0?timerPct:tp2;const ringLabel=timerSecs>0?"타이머":"오늘";if(!plan)return el("div",{style:S.pad},el("div",{style:{animation:"fadeSlideUp 0.5s ease"}},el(Logo,null)),el("div",{style:{marginTop:100,textAlign:"center",animation:"fadeSlideUp 0.6s ease 0.1s both"}},el("div",{className:"empty-float",style:{width:80,height:80,borderRadius:24,background:CARD,border:`1px solid ${BD}`,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px"}},IC.target(T3,36)),el("h2",{style:{fontSize:18,fontWeight:700,color:T2,marginBottom:8}},"아직 플랜이 없어요"),el("p",{style:{fontSize:13,color:T4,marginBottom:28}},"플랜 탭에서 새 학습 플랜을 만들어보세요"),el("button",{onClick:()=>onNav("plan"),className:"btn-glow",style:{...S.genBtn,padding:"12px 32px",width:"auto",display:"inline-flex",alignItems:"center",gap:8,fontSize:14}},IC.plus("#fff",16)," 플랜 만들기")));return el("div",{style:S.pad},el(Stagger,{delay:60},el(Logo,null),el("div",{style:{marginTop:24,marginBottom:20,display:"flex",alignItems:"center",gap:20,background:CARD,border:`1px solid ${BD}`,borderRadius:20,padding:"24px 20px"}},el(ProgressRing,{pct:ringPct,size:96,stroke:8,label:ringLabel}),el("div",{style:{flex:1}},el("h2",{style:{fontSize:18,fontWeight:700,color:T1,marginBottom:4}},"오늘의 학습"),el("p",{style:{fontSize:12,color:T3,marginBottom:timerSecs>0?4:10}},today),timerSecs>0&&el("p",{style:{fontSize:11,color:A,fontWeight:700,marginBottom:10}},`${fmtHM(timerSecs)} / ${fmtHM(planTodaySecs)} 학습 완료`),el("div",{style:{display:"flex",gap:16}},el("div",null,el("span",{style:{fontSize:20,fontWeight:700,color:T1}},td),el("span",{style:{fontSize:12,color:T3}},`/${tt}`),el("p",{style:{fontSize:10,color:T4}},"완료")),el("div",null,el("span",{style:{fontSize:20,fontWeight:700,color:T1}},pct),el("span",{style:{fontSize:12,color:T3}},"%"),el("p",{style:{fontSize:10,color:T4}},"전체"))))),el("div",{style:{display:"flex",gap:10,marginBottom:16}},[{ic:()=>IC.target(A,16),v:pct,sf:"%",l:"전체"},{ic:()=>IC.flame("#F97316",16),v:streak.streak,sf:"일",l:"스트릭"},{ic:()=>IC.zap(A,16),v:done,sf:"개",l:"완료"}].map((s,i)=>el("div",{key:i,className:"stat-card",style:{flex:1,background:CARD,border:`1px solid ${BD}`,borderRadius:22,padding:"12px 8px",display:"flex",flexDirection:"column",alignItems:"center",gap:5}},el("div",{style:{width:30,height:30,borderRadius:9,background:AS,display:"flex",alignItems:"center",justifyContent:"center"}},s.ic()),el(AnimNum,{value:s.v,suffix:s.sf,style:{fontSize:14,fontWeight:700,color:T1}}),el("span",{style:{fontSize:9,color:T3,fontWeight:600}},s.l)))),el("button",{onClick:onShare,style:{width:"100%",background:CARD,border:`1px solid ${BD}`,borderRadius:22,padding:"12px",display:"flex",alignItems:"center",justifyContent:"center",gap:8,cursor:"pointer",marginBottom:16}},IC.share(A,16),el("span",{style:{fontSize:13,fontWeight:600,color:T2}},"오늘의 공부 요약 공유")),tp?el("div",null,el("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}},el("h3",{style:{fontSize:15,fontWeight:700,color:T1}},"오늘 할 일"),el("span",{style:{fontSize:12,fontWeight:700,color:A}},`${td}/${tt}`)),el(Stagger,{delay:40},tp.tasks.map(t=>el(TaskItem,{key:t.id,task:t,onToggle:()=>onToggle(di,t.id),onEdit,locked:false})))):el("div",{style:{textAlign:"center",padding:"24px 0",color:T4,fontSize:13}},"오늘은 예정된 학습이 없어요")));}

// ═══ PLAN ═══
function PlanScreen({plan,onGenerate,onToggle,onReset,onEdit,isPro,onPaywall}){const[subjects,setSubjects]=useState([{id:1,name:"",examDate:"",scope:""}]);const add=()=>setSubjects([...subjects,{id:Date.now(),name:"",examDate:"",scope:""}]);const rm=id=>subjects.length>1&&setSubjects(subjects.filter(s=>s.id!==id));const up=(id,f,v)=>setSubjects(subjects.map(s=>s.id===id?{...s,[f]:v}:s));const ok=subjects.every(s=>s.name&&s.examDate&&s.scope);if(plan){const total=plan.reduce((s,d)=>s+d.tasks.length,0);const done=plan.reduce((s,d)=>s+d.tasks.filter(t=>t.done).length,0);const pct=total>0?Math.round((done/total)*100):0;const today=new Date().toISOString().split("T")[0];return el("div",{style:S.pad},el(Stagger,{delay:50},el("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}},el("h2",{style:{fontSize:20,fontWeight:700,color:T1}},"학습 플랜"),el("button",{onClick:onReset,className:"btn-ghost",style:S.resetBtn},"새 플랜")),el("div",{style:{background:CARD,border:`1px solid ${BD}`,borderRadius:16,padding:18,marginBottom:20}},el("div",{style:{display:"flex",justifyContent:"space-between",marginBottom:12}},el("span",{style:{fontSize:12,fontWeight:600,color:T2}},"전체 진도"),el(AnimNum,{value:pct,suffix:"%",style:{fontSize:20,fontWeight:700,color:T1}})),el("div",{style:{height:6,background:BD,borderRadius:3,overflow:"hidden"}},el("div",{className:"progress-shimmer",style:{height:"100%",borderRadius:3,background:`linear-gradient(90deg,${A2},${A},${A3})`,width:`${pct}%`,transition:"width 1s"}})),el("span",{style:{fontSize:11,color:T4,marginTop:8,display:"block"}},`${done}/${total}`)),...plan.map((day,di)=>{const isT=day.date===today;const allD=day.tasks.every(t=>t.done);const hp=day.tasks.some(t=>t.premium);const lk=hp&&!isPro;return el("div",{key:day.date,style:{background:isT?CARD2:CARD,border:`1px solid ${isT?A:BD}`,borderRadius:16,padding:16,opacity:allD?.4:1,marginBottom:2,position:"relative"}},lk&&el("div",{onClick:onPaywall,style:{position:"absolute",top:10,right:10,background:`linear-gradient(135deg,${WARN},#F97316)`,borderRadius:8,padding:"4px 10px",display:"flex",alignItems:"center",gap:4,cursor:"pointer",zIndex:2}},IC.crown("#fff",12),el("span",{style:{fontSize:10,fontWeight:700,color:"#fff"}},"PRO")),el("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,paddingBottom:10,borderBottom:`1px solid ${BD}`}},el("div",{style:{display:"flex",alignItems:"center",gap:8}},el("span",{style:{fontSize:13,fontWeight:700,color:T1}},day.date),isT&&el("span",{style:{fontSize:9,fontWeight:700,color:"#fff",background:`linear-gradient(135deg,${A},${A2})`,padding:"3px 10px",borderRadius:6}},"오늘")),el("span",{style:{fontSize:12,fontWeight:600,color:T4}},day.dayLabel)),el(Stagger,{delay:30},day.tasks.map(t=>el(TaskItem,{key:t.id,task:t,onToggle:()=>onToggle(di,t.id),onEdit,locked:t.premium&&!isPro}))));})));}return el("div",{style:S.pad},el(Stagger,{delay:70},el("h2",{style:{fontSize:20,fontWeight:700,color:T1,marginBottom:6}},"새 플랜 만들기"),el("p",{style:{fontSize:13,color:T2,marginBottom:28,lineHeight:1.6}},"과목과 시험 정보를 입력하면 AI가 최적의 학습 플랜을 생성합니다"),...subjects.map((subj,i)=>el("div",{key:subj.id,style:{background:CARD,border:`1px solid ${BD}`,borderRadius:16,padding:20,marginBottom:12}},el("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},el("span",{style:{fontSize:13,fontWeight:700,color:A}},`과목 ${i+1}`),subjects.length>1&&el("button",{onClick:()=>rm(subj.id),style:{background:"none",border:"none",cursor:"pointer",padding:4}},IC.x(T4,18))),el("div",{style:{marginBottom:14}},el("label",{style:S.label},"과목명"),el("input",{placeholder:"수학",value:subj.name,onChange:e=>up(subj.id,"name",e.target.value),style:S.input,className:"input-glow"})),el("div",{style:{marginBottom:14}},el("label",{style:S.label},"시험일"),el("input",{type:"date",value:subj.examDate,onChange:e=>up(subj.id,"examDate",e.target.value),style:S.input,className:"input-glow"})),el("div",null,el("label",{style:S.label},"시험 범위"),el("textarea",{placeholder:"1단원~3단원, 미적분, 확률과 통계",rows:3,value:subj.scope,onChange:e=>up(subj.id,"scope",e.target.value),style:S.inputArea,className:"input-glow"})))),el("button",{onClick:add,className:"btn-ghost",style:S.addBtn},IC.plus(T2,16),el("span",null,"과목 추가")),el("button",{onClick:()=>{if(ok){logEvent("plan_generate_start",{n:subjects.length});onGenerate(subjects);}},className:"btn-glow",style:{...S.genBtn,opacity:ok?1:.3,cursor:ok?"pointer":"not-allowed"}},"플랜 생성")));}

// ═══ CALENDAR ═══
function CalendarScreen({plan,onToggle,onEdit,isPro}){const[vm,setVM]=useState(()=>new Date());const[sel,setSel]=useState(null);const y=vm.getFullYear(),m=vm.getMonth();const fd=new Date(y,m,1).getDay();const dim=new Date(y,m+1,0).getDate();const today=new Date().toISOString().split("T")[0];const mn=["1월","2월","3월","4월","5월","6월","7월","8월","9월","10월","11월","12월"];const dn=["일","월","화","수","목","금","토"];const pm2={};if(plan)plan.forEach(d=>{pm2[d.date]=d;});const cells=[];for(let i=0;i<fd;i++)cells.push(null);for(let d=1;d<=dim;d++)cells.push(d);const sd=sel&&pm2[sel];const di=plan?plan.findIndex(d=>d.date===sel):-1;return el("div",{style:S.pad},el(Stagger,{delay:50},el("h2",{style:{fontSize:20,fontWeight:700,color:T1,marginBottom:20}},"캘린더"),el("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}},el("button",{onClick:()=>setVM(new Date(y,m-1,1)),style:{background:CARD,border:`1px solid ${BD}`,borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}},IC.chevL(T2,18)),el("span",{style:{fontSize:16,fontWeight:700,color:T1}},`${y}년 ${mn[m]}`),el("button",{onClick:()=>setVM(new Date(y,m+1,1)),style:{background:CARD,border:`1px solid ${BD}`,borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}},IC.chevR(T2,18))),el("div",{style:{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:8}},dn.map(d=>el("div",{key:d,style:{textAlign:"center",fontSize:11,fontWeight:600,color:T4,padding:"6px 0"}},d))),el("div",{style:{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:20}},cells.map((day,i)=>{if(!day)return el("div",{key:`e${i}`});const ds=`${y}-${String(m+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;const ht=!!pm2[ds];const it=ds===today;const is=ds===sel;const dd=pm2[ds];const ad=dd?dd.tasks.every(t=>t.done):false;return el("button",{key:i,onClick:()=>setSel(is?null:ds),style:{aspectRatio:"1",borderRadius:12,border:is?`2px solid ${A}`:`1.5px solid ${ht?BD2:"transparent"}`,background:is?AS:it?CARD2:ht?CARD:"transparent",color:it?A:ht?T1:T3,fontSize:13,fontWeight:it||is?700:500,cursor:ht?"pointer":"default",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3,transition:"all 0.2s"}},el("span",null,day),ht&&el("div",{style:{width:4,height:4,borderRadius:2,background:ad?A3:A}}));})),sd&&el("div",{style:{animation:"fadeSlideUp 0.3s ease"}},el("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}},el("h3",{style:{fontSize:15,fontWeight:700,color:T1}},sel),el("span",{style:{fontSize:12,fontWeight:600,color:A}},sd.dayLabel)),el("div",{style:{display:"flex",flexDirection:"column",gap:6}},sd.tasks.map(t=>el(TaskItem,{key:t.id,task:t,onToggle:()=>onToggle(di,t.id),onEdit,locked:t.premium&&!isPro})))),!plan&&el("div",{style:{textAlign:"center",padding:"40px 0"}},el("p",{style:{fontSize:13,color:T4}},"플랜을 생성하면 캘린더에 표시됩니다"))));}

// ═══ SETTINGS ═══
function SettingsScreen({isPro,onPaywall,profile,onSignOut}){return el("div",{style:S.pad},el(Stagger,{delay:70},el("h2",{style:{fontSize:20,fontWeight:700,color:T1,marginBottom:24}},"설정"),!isPro&&el("button",{onClick:onPaywall,style:{width:"100%",background:"linear-gradient(135deg,rgba(245,158,11,0.1),rgba(249,115,22,0.1))",border:"1.5px solid rgba(245,158,11,0.2)",borderRadius:16,padding:"18px 20px",display:"flex",alignItems:"center",gap:14,cursor:"pointer",marginBottom:16}},el("div",{style:{width:44,height:44,borderRadius:22,background:`linear-gradient(135deg,${WARN},#F97316)`,display:"flex",alignItems:"center",justifyContent:"center"}},IC.crown("#fff",20)),el("div",{style:{flex:1,textAlign:"left"}},el("span",{style:{fontSize:15,fontWeight:700,color:T1,display:"block",marginBottom:4}},"StudyFlow Pro"),el("span",{style:{fontSize:12,color:T2}},"모든 기능을 잠금 해제하세요"))),profile&&el("div",{style:{background:CARD,border:`1px solid ${BD}`,borderRadius:22,padding:16,marginBottom:8}},el("span",{style:{fontSize:12,fontWeight:600,color:T2,display:"block",marginBottom:8}},"내 프로필"),el("div",{style:{display:"flex",justifyContent:"space-between"}},el("span",{style:{fontSize:13,color:T1}},"하루 집중 시간"),el("span",{style:{fontSize:13,fontWeight:700,color:A}},`${profile.dailyHours}시간`)),profile.weakSubjects&&el("div",{style:{display:"flex",justifyContent:"space-between",marginTop:8}},el("span",{style:{fontSize:13,color:T1}},"취약 과목"),el("span",{style:{fontSize:13,fontWeight:700,color:WARN}},profile.weakSubjects))),...["알림 설정","다크/라이트 모드","데이터 초기화","버전 v1.0"].map(item=>el("div",{key:item,style:{background:CARD,border:`1px solid ${BD}`,borderRadius:22,padding:"16px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",marginBottom:8}},el("span",{style:{fontSize:14,fontWeight:600,color:T2}},item),el("div",{style:{display:"flex",alignItems:"center",gap:6}},el("span",{style:{fontSize:11,color:T4}},"Coming soon"),IC.chevR(T4,16)))),el("button",{onClick:onSignOut,style:{width:"100%",background:"rgba(239,68,68,0.06)",border:`1.5px solid rgba(239,68,68,0.15)`,borderRadius:22,padding:"15px",fontSize:14,fontWeight:700,color:DANGER,cursor:"pointer",marginTop:8}},"로그아웃")));}
...
function LoginScreen(){
  ...
  return el("div",{style:{...S.pad,minHeight:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",textAlign:"center"}},
    el("div",{style:{animation:"fadeSlideUp 0.6s ease",width:"100%",maxWidth:340}},
      el(Logo,null),
      el("h1",{style:{fontSize:24,fontWeight:700,color:T1,marginTop:32,marginBottom:12}},"환영합니다"),
      el("p",{style:{fontSize:14,color:T2,lineHeight:1.6,marginBottom:40}},"AI가 당신에게 딱 맞는 학습 플랜을 만들어드립니다"),
      el("button",{onClick:login,disabled:busy,style:{width:"100%",background:"#fff",border:"none",borderRadius:22,padding:"14px 20px",display:"flex",alignItems:"center",justifyContent:"center",gap:12,cursor:busy?"wait":"pointer",fontSize:15,fontWeight:700,color:"#111"}},
        el(GIcon,null),
...
export default function App(){
  ...
  const timerTodaySecs=timerSessions.reduce((s,x)=>s+x.duration,0);
  ...
  return el("div",{className:"sf-app",style:{...wrapStyle,position:"relative",overflowX:"hidden"}},
    ...
    el("div",{style:{paddingBottom:85}},loading?el(LoadingScreen,null):el(React.Fragment,null,
      tab==="home"&&el(HomeScreen,{plan,onToggle:toggle,onNav:setTab,onEdit:openEdit,onShare:()=>setShowShare(true),streak,timerSecs:timerTodaySecs}),
      ...
      tab==="timer"&&el(TimerScreen,{sessions:timerSessions,onSession:s=>setTimerSessions(p=>[...p,s])}),
...
const CSS=`@keyframes spin{to{transform:rotate(360deg)}}@keyframes fadeSlideUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}@keyframes popIn{from{transform:scale(0.95);opacity:0}to{transform:scale(1);opacity:1}}@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}@keyframes fadeIn{from{opacity:0}to{opacity:1}}@keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}}*{box-sizing:border-box;margin:0;padding:0}input,textarea{font-family:'Plus Jakarta Sans',sans-serif}input[type="date"]::-webkit-calendar-picker-indicator{filter:invert(1);opacity:.3}input::placeholder,textarea::placeholder{color:${T4}}button{font-family:'Plus Jakarta Sans',sans-serif}::-webkit-scrollbar{width:0}.input-glow:focus{border-color:${A}!important;outline:none}.card-hover:hover{border-color:${BD2}!important}.btn-glow:hover{opacity:.88}.btn-ghost:hover{background:${AS}!important;border-color:${A}!important;color:${A}!important}.stat-card:hover{border-color:${BD2}!important}.empty-float{animation:float 3s ease-in-out infinite}.logo-pulse{}.ring1{animation:spin 3s linear infinite}.ring2{animation:spin 4s linear infinite reverse}.ring3{animation:spin 5s linear infinite}.progress-shimmer{position:relative}@media (max-width:768px){.sf-app{width:100%;max-width:100%}.sf-nav{width:100%;max-width:100%;left:0;transform:none}}@media (min-width:769px){.sf-app{max-width:500px;margin:0 auto}.sf-nav{max-width:500px;left:50%;transform:translateX(-50%)}}`;


const S={pad:{padding:"20px 20px 32px"},nav:{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",zIndex:100},navInner:{display:"flex",justifyContent:"space-around",alignItems:"center",background:`${CARD}f2`,borderTop:`1px solid ${BD}`,padding:"8px 0 14px",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)"},navItem:{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,position:"relative"},label:{display:"block",fontSize:12,fontWeight:600,color:T2,marginBottom:8},input:{width:"100%",background:BG2,border:`1.5px solid ${BD}`,borderRadius:20,color:T1,padding:"13px 16px",fontSize:14,transition:"border-color 0.2s ease"},inputArea:{width:"100%",background:BG2,border:`1.5px solid ${BD}`,borderRadius:20,color:T1,padding:"13px 16px",fontSize:14,resize:"vertical",transition:"border-color 0.2s ease"},addBtn:{width:"100%",background:"none",border:`1.5px dashed ${BD}`,borderRadius:22,color:T2,padding:"14px",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:12},genBtn:{width:"100%",background:`linear-gradient(135deg,${A},${A2})`,border:"none",borderRadius:22,color:"#fff",padding:"15px",fontSize:15,fontWeight:700,cursor:"pointer"},resetBtn:{background:"transparent",border:`1.5px solid ${BD}`,borderRadius:22,color:T2,padding:"8px 18px",fontSize:12,fontWeight:600,cursor:"pointer"}};
